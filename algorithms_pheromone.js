function aco(settings) {
	...
	while(antCount > 0) {
		...
		// начало цикла одного муравья, который проходит по всем вершинам
		...
		//checkAvailability(currentVertex);
			...
			if(currentVertex == stop) {
				...
				// дальше считаем длину пути (нужно для уровня феромона) 104-119
	...
	if(pathFound) {
		output += toHtml('Путь: '+pathString + '(длина пути: '+length+')');
